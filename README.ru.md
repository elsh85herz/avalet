...
- **Avalet без ключей**: пробный период (500 000 токенов, без карты), потом подписка Pro на месяц. Запросы идут через сервер Avalet, который считает токены и не хранит расшифровки. *Пока не запущено: сервер ещё в работе (см. "Известные ограничения").*

[English version](README.md)

> Статус: релиз-кандидат 0.2.0-rc.1. Сделано практикующим системным аналитиком в первую очередь для своих встреч. Пока только macOS.

## Что умеет

- **Первый запуск с подсказками.** Четыре коротких экрана: разрешения (с живым статусом и кнопкой "Как исправить"), доступ (Avalet или свой ключ с настоящей проверкой), загрузка модели распознавания (прогресс, отмена, докачка) и контекст встречи с 20-секундной проверкой, которая показывает настоящую подсказку ещё до первого звонка.
- **Просто по умолчанию, всё остальное в расширенном режиме.** Простой режим: Старт/Пауза, тип встречи, контекст, расшифровка, итог и экспорт. В расширенном режиме (Настройки, "Расширенный режим") все настройки провайдеров, модели, живой чек-лист, текст со скриншотов и логи.
...
6. Запустите Avalet и пройдите настройку: разрешите **Микрофон** и **Запись экрана** (через запись экрана macOS разрешает захват звука собеседника), выберите свой ключ или Avalet и скачайте модель распознавания (около 0,5 ГБ, один раз, с Hugging Face).
...
|---|---|---|
| ![Шаг доступа](docs/screens/wizard-2-access-trial-dark.png) | ![Встреча с итогом](docs/screens/simple-meeting-summary-dark.png) | ![Подсказка](docs/screens/overlay-simple-suggestion-dark.png) |
...
- **Вариант "Avalet без ключей" пока не запущен.** Клиентская часть готова и проверена на тестовом сервере; сервер оплаты, его ключ подписи и платёжный шлюз ещё не развёрнуты. До тех пор используйте свой ключ.
